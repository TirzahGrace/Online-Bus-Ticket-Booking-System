...
function lockSuggestion(evt) {
    ...
    if (searchInput.id === "routeSearch") {
        // Set the route_id & depDate
        console.log(evt.target.dataset.id);
        console.log(evt.target.dataset.deptiming);
        ...
        }//filling the input bar values with the source and destinations    
        ...
        const route_busNo = routeData.find(({ route_id: id }) => {
            return id === route_id
        }).bus_no;//bus no of the route selected

        console.log(seatData.find(({ bus_no }) => {
            console.log('hello');
            console.log(bus_no, route_busNo);
            return bus_no === route_busNo;
        }));//printing the buses in the database which have the same bus no as the route selected
        ...
        }//adding notAvailable as a class for that seat
    }
    // This is default
